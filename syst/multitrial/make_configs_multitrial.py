...
def get_reference_config_pt_bin(config_flow, iPtBin):
    ...
    pt_bin_config['v2extraction']['Rebin'] = [pt_bin_config['v2extraction']['Rebin'][iPtBin]]
    ...
    for _, setting in enumerate(cfg_mod['ptbins']):
        for ptmin, ptmax in setting['ranges']:
            ...
            for idx, variant in enumerate(config_variants):
                ...
                if variant.get("InvMassBinSteps"):
                    cfg_variant["projections"]["inv_mass_bins"] = [np.arange(variant['MassMin'], variant['MassMax'] + variant['InvMassBinSteps'],
