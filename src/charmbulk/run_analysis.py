...
def main():
    ...
    # ════════════════════════════════════════════════════════════════════
    # Node 2: proj_data / proj_mc  (projections)
    # ════════════════════════════════════════════════════════════════════
    if operations.get("proj_data", False) or operations.get("proj_mc", False):
        logger("Step 2: Projecting distributions...", "INFO")

        # Find the first cutset config (generated by make_yaml in step 1)
        ...
        if not cutset_files:
            logger(f"No cutset files found in {cutset_dir}, using default path", "WARNING")
            cutset_config = os.path.join(cutset_dir, "cutset_00.yml")
        else:
            cutset_config = os.path.join(cutset_dir, cutset_files[0])
